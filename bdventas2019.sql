...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 24-05-2019 a las 12:07:13
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarid` (IN `nombre2` VARCHAR(20))  NO SQL
BEGIN
SELECT persona.dni,persona.idpersona FROM persona WHERE persona.nombres=nombre2;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarpersona2` (IN `idpersona2` INT)  NO SQL
BEGIN
SELECT persona.nombres,persona.dni FROM persona WHERE persona.idpersona=idpersona2;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarPorductoCodigo` (IN `cod` INT(11))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateuser` (IN `idpersona2` INT, IN `iduser2` INT, IN `apellidoper` VARCHAR(20), IN `nombreper` VARCHAR(20), IN `clave2` VARCHAR(20))  NO SQL
BEGIN
update usuario,persona set usuario.clave=clave2,persona.nombres=nombreper,persona.apellidos=apellidoper where usuario.idusuario=iduser2 AND persona.idpersona=idpersona2;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `usuariobloqueado` (IN `user2` VARCHAR(20))  NO SQL
BEGIN
update usuario SET usuario.estado=0 where usuario.nom_user=user2;
END$$
...
(1, 'Laptop', 2500, 14, 101),
(2, 'Pantalla Dell', 1000, 19, 102),
(3, 'Parlantes', 100, 20, 103);
